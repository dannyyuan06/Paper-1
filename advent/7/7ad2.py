...
def replace(a: str):
  ...
  arr = [a]
  for key in keys:
    if key != 'J': arr.append(a.replace('J', key))
  return arr
...
def compare(a:str, b:str):
  ...
  a_type = 0
  b_type = 0
  a_combinations = replace(a)
  b_combinations = replace(b)
  for comb in a_combinations:
    for i in range(len(types)):
      type = types[i]
      if type(comb) and i > a_type: a_type = i
  for comb in b_combinations:
    for i in range(len(types)):
      type = types[i]
      if type(comb) and i > b_type: b_type = i
  if a_type == b_type:
