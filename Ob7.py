...
class Pattern():
    ...
    def MatchesPattern(self, PatternString, SymbolPlaced):
        ...
        for i in range(4):
            ...
            Circle = Pattern[:-1]
            PatternCheck = Circle[i*2:] + Circle[:i*2] + Pattern[-1]
            for Count in range(0, len(PatternCheck)):
                try:
                    if PatternCheck[Count] == self.__Symbol and PatternString[Count] != self.__Symbol:
                        CorrectCounter -= 1
                        break
